<x-layouts.app>
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-white">{{ __('ui.user.create') }}</h1>
        </div>
        ...
        <div class="card">
            <form method="POST" action="{{ route('users.store') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <select id="role_id" name="role_id" class="input-field" required>
                        @foreach($roles as $role)
                            <option value="{{ $role->id }}" data-slug="{{ $role->slug }}" {{ (string)old('role_id') === (string)$role->id ? 'selected' : '' }}>
                                {{ $role->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                @php
                    $createSelectedRole = $roles->firstWhere('id', old('role_id')) ?? $roles->first();
                    $createShowPerms = $createSelectedRole && $createSelectedRole->slug !== 'administrator';
                @endphp
                <div id="vm-permissions-section" class="space-y-3 p-4 rounded-lg bg-slate-800/50 border border-slate-600"
                    style="display: {{ $createShowPerms ? 'block' : 'none' }}">
                    ...
                    <div class="grid grid-cols-2 gap-x-8 gap-y-3">
                        <label class="flex items-center gap-2 cursor-pointer">
                            ...
                            <input type="checkbox" name="can_create_vm" value="1" class="rounded border-slate-600"
                                {{ old('can_create_vm', true) ? 'checked' : '' }}>
                        ...
                        <label class="flex items-center gap-2 cursor-pointer">
                            ...
                            <input type="checkbox" name="can_delete_vm" value="1" class="rounded border-slate-600"
                                {{ old('can_delete_vm', true) ? 'checked' : '' }}>
                        ...
                        <label class="flex items-center gap-2 cursor-pointer">
                            ...
                            <input type="checkbox" name="can_edit_others_vm" value="1" class="rounded border-slate-600"
                                {{ old('can_edit_others_vm', false) ? 'checked' : '' }}>
                        ...
                        <label class="flex items-center gap-2 cursor-pointer">
                            ...
                            <input type="checkbox" name="can_delete_others_vm" value="1" class="rounded border-slate-600"
                                {{ old('can_delete_others_vm', false) ? 'checked' : '' }}>
                ...
                <div class="flex justify-end gap-4">
                    ...
                    <button type="submit" class="btn-primary">{{ __('ui.create') }}</button>
                </div>
            </form>
        </div>
    </div>
